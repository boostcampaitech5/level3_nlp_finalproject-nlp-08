...
def BERT_infer(input):
    ...
    vector_data = load_vector_data(base_path +'/../data/law_data/law_data_drop_vector.bin')
    ...
    for i,index in enumerate(top_question):
        if data_cosine[i] >= 0.5:
            #print(f'유사 판례 case{i} 유사도 점수 : {data_cosine[i]}')
            #print('caseNumber :', text_data[index][0])
            #print('courtName :', text_data[index][1])
            #print('caseName :', text_data[index][3])
            #print('judgementAbstract :', text_data[index][4])
            #print('refArticle :', text_data[index][7])
            #print('judgementNote :', text_data[index][9])
            #print()
            pan_list.append(f"case Number : {text_data[index][0]} judgementAbstract : {text_data[index][4]} judgementNote :{text_data[index][9]}")
