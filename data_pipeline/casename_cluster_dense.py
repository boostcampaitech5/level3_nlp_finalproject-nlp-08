...
def get_dense_embeddings(data):
    model_name = "snunlp/KR-Medium"  # Replace with the Hugging Face model you want to use
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModel.from_pretrained(model_name)
    
    encoded_inputs = tokenizer(data, padding=True, truncation=True, max_length=512, return_tensors="pt")
    with torch.no_grad():
        outputs = model(**encoded_inputs)
        embeddings = outputs.last_hidden_state.mean(dim=1)  # Average pooling to get document embeddings
...
def predict_cluster(new_document, tokenizer, model, kmeans_model):
    encoded_input = tokenizer(new_document, padding=True, truncation=True, max_length=512, return_tensors="pt")
    with torch.no_grad():
        output = model(**encoded_input)
        new_embedding = output.last_hidden_state.mean(dim=1)
...
if __name__ == "__main__":
    ...
    dense_embeddings, tokenizer, model = get_dense_embeddings(tokenized_documents)

    distances_cosine = pairwise_distances(dense_embeddings, metric="cosine")
    distances_euclidean = pairwise_distances(dense_embeddings, metric="euclidean")
    ...
    embeddings_pca = pca.fit_transform(dense_embeddings)

    predicted_cluster, distance_to_center = predict_cluster(query_document, tokenizer, model, kmeans_cosine)
